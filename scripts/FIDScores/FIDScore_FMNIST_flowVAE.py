...
param_file   = 'params_fmnist_-1_40_infoGAN_VAE_best_params_noaugment_simple_flow_prior_full_sigmaVAE_beta1'
...
print(params['plot_dir'], params['module_dir'])
...
for ii in range(50):
    ...
    fid_scores_rec.append(evaluate_fid_score(sample_1,sample_2))
    
fid_scores_rec=np.asarray(fid_scores_rec)
_ = plt.hist(fid_scores_rec, density=True, bins=100)
plt.show()
print('recs:', np.mean(fid_scores_rec),np.std(fid_scores_rec))
...
#    sample_2 = resample(x_valid,replace=True,n_samples=10000)
#    fid_scores_nvp.append(evaluate_fid_score(sample_1,sample_2))
    
#fid_scores_nvp=np.asarray(fid_scores_nvp)
#_ = plt.hist(fid_scores_nvp, density=True, bins=100)
#plt.show()
#print('samples', np.mean(fid_scores_nvp),np.std(fid_scores_nvp))
